...
RegisterNetEvent('QBCore:Client:PedLoaded')
AddEventHandler('QBCore:Client:PedLoaded', function()
	if not playerLoaded then
		playerLoaded = true
		CreateBlip()
	end
...
RegisterNetEvent("erp-trucking-cl:finishTruckSpawn")
AddEventHandler("erp-trucking-cl:finishTruckSpawn", function(netId, model, location)
    print("Waiting for net id to exist", netId)
    local startTime = GetGameTimer()
	while not NetworkDoesNetworkIdExist(netId) do
        if GetGameTimer() - startTime >= 1250 then
            TriggerServerEvent("erp-garage-sv:spawnVehicle", model, location, true, "erp-trucking-cl:finishTruckSpawn")
            return
        end
		Citizen.Wait(100)
	end
	...
	TriggerEvent("vehiclekeys:client:SetOwner", vin)
    QBCore.Functions.Notify('You received keys to the vehicle.', 1)
end)

AddEventHandler('erp-trucking-cl:startSoloJobUnowned', function()
	if (GetNetworkTime() - missionStartTime) >= 150000 or GetNetworkTime() <= 300000 then
		...
		while not QBCore.Functions.SpawnClear(vector3(truckSpawnPos.x, truckSpawnPos.y, truckSpawnPos.z), 4.5) do
		...
		if truck == nil or not DoesEntityExist(truck) then
			...
			if (playerVehicleModel ~= 'phantom' or (playerVehicleModel == 'phantom' and GetEntityHealth(playerVehicle) <= 0.0)) and (playerVehicleModel ~= 'w900' or (playerVehicleModel == 'w900' and GetEntityHealth(playerVehicle) <= 0.0)) and (playerVehicleModel ~= 'phantom3' or (playerVehicleModel == 'phantom3' and GetEntityHealth(playerVehicle) <= 0.0)) and (playerVehicleModel ~= 'hauler' or (playerVehicleModel == 'hauler' and GetEntityHealth(playerVehicle) <= 0.0)) then
				...
				end, GetHashKey('phantom'), truckSpawnPos, false)
				--TriggerServerEvent("erp-garage-sv:spawnVehicle", GetHashKey('phantom'), truckSpawnPos, true, "erp-trucking-cl:finishTruckSpawn")
				while truck == nil do
					Citizen.Wait(100)
				end
				exports['LegacyFuel']:SetFuel(truck, 100.0)
				TriggerEvent("vehiclekeys:client:SetOwner", Entity(truck).state.VIN)
				doingARun = true
				Citizen.CreateThread(function()
					healthMonitor()
				end)
...
AddEventHandler('erp-trucking-cl:attemptToStartGroupJob', function()
	local group = exports["erp-assets"]:getGroup()
	if group ~= nil then
		...
		if not memberTooFar then
			if group.memberCount > 1 and group.memberCount < 5 then
				TriggerServerEvent("erp-trucking-sv:startGroupJob", group)
			else
...
RegisterNetEvent("erp-trucking-cl:startGroupJob")
AddEventHandler("erp-trucking-cl:startGroupJob", function(sentTruckGroup)
	...
	while not QBCore.Functions.SpawnClear(truckSpawnPos, 4.5) do
		truckSpawnPos = truckSpawnSpots[math.random(1, #truckSpawnSpots)]
		Citizen.Wait(5)
	end
	local truckSpawnHeading = truckSpawnSpots.w
	...
	if truck == nil or not DoesEntityExist(truck) then
		...
		if (playerVehicleModel ~= 'phantom' or (playerVehicleModel == 'phantom' and GetEntityHealth(playerVehicle) <= 0.0)) and (playerVehicleModel ~= 'phantom3' or (playerVehicleModel == 'phantom3' and GetEntityHealth(playerVehicle) <= 0.0)) then
			...
			if vin == nil then
				print("Check 4")
				TriggerServerEvent("erp-vehRegistration-sv:genVehVIN", NetworkGetNetworkIdFromEntity(truck), false)
				while vin == nil do
			...
			--QBCore.Functions.Notify("Looks like you didn't bring your own truck. Here use one of mine.",1, 15000)
			SendNotification("Looks like you didn't bring your own truck. Here use one of mine.")
			missionStartTime = GetNetworkTime()
			ownTruck = false
			TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "truckSpawn", {id = identifier})
			...
			ownTruck = true
			deliveriesBeforeReturning = truckGroup.numberOfDropOffs
			--QBCore.Functions.Notify("I see you got money and brought your own truck. Cool.",1, 15000)
			SendNotification("I see you got money and brought your own truck. Cool.")
			TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "truckSpawn", {id = identifier})
			while truckGroup.stage == 0 and not cancelMission do
				Citizen.Wait(100)
...
RegisterNetEvent("erp-trucking-cl:updateTruckGroup")
AddEventHandler("erp-trucking-cl:updateTruckGroup", function(sentTruckGroup)
	truckGroup = sentTruckGroup
...
AddEventHandler('erp-trucking-cl:returnTruck', function()
	if DoesEntityExist(truck) then
		if not ownTruck then
			...
			if not DoesEntityExist(truck) then
				truck = nil
				--QBCore.Functions.Notify("Successfully returned the truck.",3, 15000)
				SendNotification("Successfully returned the truck.")
				TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "missionCancel", nil)
				resetMission()
			else
				--QBCore.Functions.Notify("You were not able to return the truck. Make sure no one is in it.",2, 15000)
			...
			--QBCore.Functions.Notify("You stopped truckin' for the day", 1, 15000)
			SendNotification("You stopped truckin' for the day")
			TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "missionCancel", nil)
			resetMission()
		end
	else
		--QBCore.Functions.Notify("It seems you don't have a truck to return",2, 10000)
		SendNotification("It seems you don't have a truck to return")
	end
end)

AddEventHandler('erp-trucking-cl:collectPay', function()
	TriggerServerEvent('erp-trucking-sv:pay', QBCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('erp-trucking-cl:resetTotalPay')
AddEventHandler('erp-trucking-cl:resetTotalPay', function()
	totalJobPay = 0
end)

RegisterNetEvent('erp-trucking-cl:setTotalPay')
AddEventHandler('erp-trucking-cl:setTotalPay', function(totalPay)
	totalJobPay = totalPay
	if totalJobPay > 0 then
		--QBCore.Functions.Notify("You have $" .. totalJobPay .. " waiting to be collected at the trucking location", 1, 15000)
...
function newDelivery()
	if trailer == nil then
		...
		--QBCore.Functions.Notify("A trailer is ready for pickup and has been marked on your map",4, 10000)
		...
		while #(GetEntityCoords(GetPlayerPed(-1)) - currentPickupSpot[1]) > 100.0 and not cancelMission do
			Citizen.Wait(500)
			if count >= 600 then
				--QBCore.Functions.Notify("You need to go to the marked location on your map to pickup your trailer",4, 10000)
		...
		QBCore.Functions.TriggerCallback("QBCore:Server:SpawnVehicle", function(netId)
			trailer = NetworkGetEntityFromNetworkId(netId)
			...
			local vin = nil
			if vin == nil then
				TriggerServerEvent("erp-vehRegistration-sv:genVehVIN", NetworkGetNetworkIdFromEntity(trailer), false)
				while vin == nil do
					vin = Entity(trailer).state.VIN
					Citizen.Wait(10)
				end
			end
			exports['LegacyFuel']:SetFuel(trailer, 100.0)
			TriggerEvent("vehiclekeys:client:SetOwner", vin)
		end, trailerModel, vector4(currentPickupSpot[1], currentPickupSpot[2]), false)
		...
		while (not IsEntityAttached(trailer) and GetEntityAttachedTo(trailer) ~= truck) and not cancelMission do
			Citizen.Wait(500)
			if count >= 600 then
				--QBCore.Functions.Notify("You need to attach the marked trailer to your truck",4, 10000)
		...
		TriggerServerEvent("erp-trucking-sv:abc", currentDropOffSpot[1])
		...
		--QBCore.Functions.Notify("Deliver the contents of the trailer to the marked location on your map",4, 15000)
		...
		while #(GetEntityCoords(trailer) - currentDropOffSpot[1]) > 65.0 and not cancelMission do
			Citizen.Wait(500)
			if count >= 600 then
				--QBCore.Functions.Notify("You need to go to the marked location on your map to make a drop off",4, 15000)
		...
		QBCore.Functions.Notify("Back the back of the trailer up to the marker to make the drop off",4, 15000)
		...
		while (#(rearCoords - currentDropOffSpot[1]) > 1.0 or math.abs(((GetEntityHeading(trailer) - currentDropOffSpot[2])  + 180) % 360 - 180) > 35) and not cancelMission do
			Citizen.Wait(500)
			if count >= 600 then
				QBCore.Functions.Notify("You need to go to the marked location on your map to make a drop off",4, 10000)
				count = 0
			else
				count = count + 1
			end
		...
		inPosition = true
		if DoesBlipExist(missionblip) then RemoveBlip(missionblip) end
		QBCore.Functions.Notify("Get out of the truck and go to the rear to help unload the contents",4, 15000)
		zOffset = -2.75
		if truckType == 6 then
			zOffset = -1.20
		end
		...
		if exports["erp-assets"]:getTruckingBoost() - GetGameTimer() > 0 then
			local extra = 0
			extra = math.floor((currentJobPay * 0.20) + 0.5)
			QBCore.Functions.Notify('You received extra $' .. extra .. ' due to using something that gives you a boost', 109)
			QBCore.Functions.Notify('You have ' .. math.floor((exports["erp-assets"]:getTruckingBoost()- GetGameTimer()) / 1000 + 0.5) .. "s left to your boost", 423122)
		...
		TriggerServerEvent('erp-dailies-server:handleDailyEvent', 'truckdelivery', 1, GetEntityCoords(GetPlayerPed(-1), false), 10069)
		local charSteam = QBCore.Functions.GetPlayerData().citizenid
		TriggerServerEvent('erp-trucking-sv:addToPayslip', charSteam, currentJobPay)
		if 1 ~= deliveryCount and not cancelMission then
			newDelivery()
		else
			QBCore.Functions.Notify("Return the trailer to where you got it from. A marker has been placed on your map",4, 15000)
			TriggerServerEvent("erp-trucking-abc", currentPickupSpot[1])
			...
			while #(GetEntityCoords(trailer) - currentPickupSpot[1]) > 50.0 and not cancelMission do
				Citizen.Wait(500)
				if count >= 600 then
					QBCore.Functions.Notify("You need to go to the marked location on your map to return the trailer",4, 15000)
					count = 0
				else
					count = count + 1
				end
			...
			QBCore.Functions.Notify("Put the trailer in the spot to complete the run.",4, 15000)
			inPosition = false
			Citizen.CreateThread(function()
				while not inPosition and not cancelMission do 
			...
			DeleteEntity(trailer)
			trailer = nil
			QBCore.Functions.Notify("Successfully did a full run. Extra cash was added to your total payment. Go get it from the starting area",3, 15000)
			QBCore.Functions.Notify("Either return the truck or start another mission",4, 15000)
			TriggerServerEvent('erp-trucking-sv:addToPayslip', QBCore.Functions.GetPlayerData().citizenid, math.random(125, 250))
			resetMission()
		end
	else
		local newDropOffSpot = math.random(1, #Config.DeliveryLocations[lastPickupSpot])
		...
		totalJobPay = totalJobPay + currentJobPay
		TriggerServerEvent('erp-dailies-server:handleDailyEvent', 'truckdelivery', 1, GetEntityCoords(GetPlayerPed(-1), false), 10069)
		TriggerServerEvent('erp-trucking-sv:addToPayslip', QBCore.Functions.GetPlayerData().citizenid, currentJobPay)
...
function handleTrailerSpawn()
	...
	--QBCore.Functions.Notify("Attach to the trailer with the marker above it",4, 10000)
	SendNotification("Attach to the trailer with the marker above it")
	count = 0
	local attached = false
	Citizen.CreateThread(function()
	...
	TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "trailerAttached", {id = playerId})
...
function newGroupDelivery()
	...
	TriggerServerEvent("erp-trucking-sv:abc", currentDropOffSpot[truckGroup.personData[playerId].trailerSpawn][1])
	...
	print(startSpot, distance, playerId, exports["erp-assets"]:getGroup().creator, currentDropOffSpot[truckGroup.personData[playerId].trailerSpawn][1])
	if playerId == exports["erp-assets"]:getGroup().creator then
		TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "distance", {dist = distance})
	end
	--QBCore.Functions.Notify("Deliver the contents of the trailer to the marked location on your map",4, 15000)
	SendNotification("Deliver the contents of the trailer to the marked location on your map")
	local count = 0
	...
	inPosition = true
	TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "docked", {id = playerId})
	...
	rearCoords = GetOffsetFromEntityInWorldCoords(trailer,0.0,vehLength,0.0)
	UnloadTruck(rearCoords)
	TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "deliveryMade", {id = playerId})
	...
	totalJobPay = totalJobPay + currentJobPay
	TriggerServerEvent('erp-dailies-server:handleDailyEvent', 'truckdelivery', 1, GetEntityCoords(GetPlayerPed(-1), false), 10069)
	TriggerServerEvent('erp-trucking-sv:addToPayslip', playerId, currentJobPay, truckGroup.numberOfMembers)
	TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "receivedPay", {id = playerId})
	...
	if truckGroup.stage == 2 then
		TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "resetDeliveriesMade", {id = playerId})
		newGroupDelivery()
	else
		QBCore.Functions.Notify("Return the trailer to where you got it from. A marker has been placed on your map",4, 15000)
		...
		--QBCore.Functions.Notify("Waiting for the rest of your group to go to their locations",1, 15000)
		...
		DeleteEntity(trailer)
		trailer = nil
		TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "returnTruck", {id = playerId})
		Citizen.Wait(1000)
		...
		TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "readyToComplete", {id = playerId})
		--QBCore.Functions.Notify("Successfully did a full run. Extra cash was added to your total payment. Go get it from the starting area",3, 15000)
		...
		TriggerServerEvent('erp-trucking-sv:addToPayslip', playerId, math.random(200, 375), truckGroup.numberOfMembers)
		resetMission()
	end
end
...
function UnloadTruck(coord)
	...
	local finished = 0
	QBCore.Functions.Progressbar("work_carrybox", "UNLOADING TRAILER", math.random(10000,15000), false, false, {
...
function healthMonitor()
	while not cancelMission and doingARun do
		--if GetEntityHealth(truck) <= 0.0 or not DoesEntityExist(truck) or exports["erp-ambulancejob"]:GetDeath() then
		if GetEntityHealth(truck) <= 0.0 or not DoesEntityExist(truck) then
			cancelMission = true
			Citizen.Wait(1000)
			resetMission()
			if doingGroupJob then
				TriggerServerEvent("erp-trucking-sv:updateTruckGroup", exports["erp-assets"]:getGroup(), "missionCancel", {})
			end
		end
